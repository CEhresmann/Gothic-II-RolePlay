...
function isPlayerNPC(pid) {
    return isRemoteNpc(pid) || isLocalNpc(pid)
}
...
function PlayerList::init()
{
	...
	_hostname = Label(0, 0, CFG.Hostname)
...
function PlayerList::insert(pid) {
    if (suppressNpcNames && isPlayerNPC(pid)) return
...
function PlayerList::setSuppressNpcNames(suppress) {
    suppressNpcNames = suppress
    ...
    foreach (pid in getPlayers()) {
        if (!suppressNpcNames || !isPlayerNPC(pid)) {
            insert(pid)
        }
    }
...
{
	PlayerList.init()
	PlayerList.suppressNpcNames = true
})
