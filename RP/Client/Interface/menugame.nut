...
Interface.MenuGame <- {
	window = GUI.Window({
        ...
        sizePx = {width = 0.20 * Resolution.x, height = 0.375 * Resolution.y},
...
Interface.MenuGame.bindsButton <- GUI.Button({
		relativePositionPx = {x = 0.01 * Resolution.x, y = 0.23 * Resolution.y},
        sizePx = {width = 0.18 * Resolution.x, height = 0.04 * Resolution.y},
		file = "DLG_CONVERSATION.TGA"
		label = {text = _L("Binds")}
		collection = Interface.MenuGame.window
...
Interface.MenuGame.exitButton <- GUI.Button({
		relativePositionPx = {x = 0.01 * Resolution.x, y = 0.31 * Resolution.y},
        sizePx = {width = 0.18 * Resolution.x, height = 0.04 * Resolution.y},
		file = "DLG_CONVERSATION.TGA"
		label = {text = _L("Exit")}
		collection = Interface.MenuGame.window
...
addEventHandler("GUI.onClick", function(self)
{
	...
	switch (self)
	{
        ...
        break;
		case Interface.MenuGame.bindsButton:
            Interface.MenuGame.hide();
            Interface.Binds.show();
        break;
...
addEventHandler("onChangeLanguage", function(lang) {
    ...
    Interface.MenuGame.bindsButton.setText(_L("Binds"));
