...
Interface.TradeRequestMenu <- {
    ...
    "show" : function(traderName) {
        ...
        this.Topic.setRelativePositionPx((0.472 * Resolution.x - textWidthPx(topictradetext)) / 2, this.Topic.getRelativePositionPx().y);
		
		local tradertext = traderName + " chce handlowaæ z Tob¹"
        this.Details.setText(tradertext);
        this.Details.setRelativePositionPx((0.472 * Resolution.x - textWidthPx(tradertext)) / 2, this.Details.getRelativePositionPx().y);
        ...
        if (subId == PacketPlayer.Trade) {
            ...
            switch (packetType) {
                ...
                case 2:
                    ...
                    if (targetId == heroId) {
                        if (accepted) {
                            //sendClientChatMessage(getPlayerName(responderId) + " zaakceptowa³ handel");
                        }
                        Interface.TradeRequestMenu.traderId = -1;
                    }
                    break;
...
addEventHandler("onKeyDown", function(key) {
    if (key == KEY_RSHIFT && focusPlayer != -1) {
...
function showPlayerDialog() {
    ...
    if (isLocalNpc(focusPlayer)) return;
