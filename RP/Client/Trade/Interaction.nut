local focusDraw = Label(0, 0, "RSHIFT - Interakcja");
local focusPlayer = -1;
...
Interface.InteractMenu.Topic <- GUI.Label({
...
Interface.TradeRequestMenu <- {
    ...
    "show" : function(traderName) {
        ...
        local topictradetext = "Prośba o handel"
        ...
        local tradertext = traderName + " chce handlować z Tobą"
...
addEventHandler("onKeyDown", function(key) {
    if (key == KEY_RSHIFT && focusPlayer != -1 && isPlayerCreated(focusPlayer) && !isLocalNpc(focusPlayer) && getPlayerName(focusPlayer) != "null" && !chatInputIsOpen()) {
...
function showPlayerDialog() {
    if (focusPlayer == -1 || !isPlayerCreated(focusPlayer) || isLocalNpc(focusPlayer) || getPlayerName(focusPlayer) == "null" && !chatInputIsOpen()) return hidePlayerDialog();
