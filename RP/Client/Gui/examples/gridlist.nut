...
local addPlayerColumnFirstCellButton = GUI.Button({
	...
	sizePx = {width = 300, height = 40}
	file = "MENU_INGAME.TGA"
	label = {text = "Add Player Column first cell"}
	collection = window
})
...
local addPingColumnFirstCellButton = GUI.Button({
	...
	sizePx = {width = 300, height = 40}
	file = "MENU_INGAME.TGA"
	label = {text = "Add Ping Column first cell"}
	collection = window
})
...
local removePlayerColumnFirstCellButton = GUI.Button({
	...
	sizePx = {width = 300, height = 40}
	file = "MENU_INGAME.TGA"
	label = {text = "Remove Player Column first cell"}
	collection = window
})
...
local removePingColumnFirstCellButton = GUI.Button({
	...
	sizePx = {width = 300, height = 40}
	file = "MENU_INGAME.TGA"
	label = {text = "Remove Ping Column first cell"}
	collection = window
})
...
local removePlayerColumnButton = GUI.Button({
	...
	sizePx = {width = 300, height = 40}
	file = "MENU_INGAME.TGA"
	label = {text = "Remove Player Column"}
	collection = window
})
...
addEventHandler("GUI.onClick", function(self)
{
	...
	foreach (cell in visibleRow.cells)
		cell.label.setColor({r = randomChar(), g = randomChar(), b = randomChar()})
})
...
addEventHandler("onInit", function()
{
	...
	for(local i = 1; i <= 26; i++)
	{
		local row = gridlist.addRow(
			{text = rawstring(ASCIILetter.tochar()), file = "INV_SLOT_EQUIPPED.TGA"},
			{text = randomChar(), file = "INV_SLOT_EQUIPPED.TGA"}
		)

		foreach (cell in row.cells)
			cell.setLabelColor({r = randomChar(), g = randomChar(), b = randomChar()})
