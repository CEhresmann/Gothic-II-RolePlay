...
#private:
	...
	constructor(arg = null)
	{
		...
		_placeHolder = ("placeHolder" in arg) ? (arg.placeHolder) : (label != null ? label.getText() : _placeHolder)
	...
	function updateListSize()
	{
		...
		if (list._visibleRowsCount != 0)
			GUI.Sprite.setSizePx.call(list, sizePx.width, ((list.getRowHeightPx() + list.getRowSpacingPx()) * list._visibleRowsCount) + (margin.top + margin.bottom))
		else
			GUI.Sprite.setSizePx.call(list, sizePx.width, 0)
