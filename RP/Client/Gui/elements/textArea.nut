...
local VisibleLine = class extends GUI.Draw
{
#public
	id = -1
...
local GUITextAreaClasses = classes(GUI.Texture, GUI.Margin, GUI.Alignment)
...
#private:
	...
	function _reloadText(newText, updateTooLongText, updateScrollbarValue)
	{
		...
		for (local letterId = updateLine.start, textLen = newText.len(); letterId < textLen; ++letterId)
		{
			...
			local letter = letterAscii.tochar().tostring()
	...
	function _goToPrevWord()
	{
		...
		// Pass separators.
		while (i > 0 && _text[i].tochar().tostring() == _separator)
			--i

		// Pass letters until first separator.
		while (i > 0 && _text[i - 1].tochar().tostring() != _separator)
			--i
	...
	function _goToNextWord()
	{
		...
		// Pass letters until first separator.
		while (i < textLen && _text[i].tochar().tostring() != _separator)
			++i
	...
	function _goVertically(direction)
	{
		...
		while (true)
		{
			...
			if (prevId < _cursor)
			{
				prevWidthPx += textWidthPx(_text[prevId].tochar().tostring())
				++prevId
			}
			...
			if (newId < newLen)
			{
				newWidthPx += textWidthPx(newText[newId].tochar().tostring())
				++newId
			}
	...
	function _moveCursorByClick()
	{
		...
		for (local end = text.len(); i < end; ++i)
		{
			...
			lineWidth += textWidthPx(text[i].tochar().tostring())  * _scale
		}
