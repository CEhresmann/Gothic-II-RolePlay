...
addEvent("onClientLPUpdate");
addEvent("onClientProfessionsUpdate");

function getLocalPlayerLearningPoints()
{
    return Player.learningPoints;
}

/**
 * Zwraca lokalny poziom profesji gracza (do u¿ycia w GUI).
 * @param {integer} professionId - ID profesji.
 * @returns {integer} - Poziom profesji.
 */
function getLocalPlayerProfessionLevel(professionId)
{
    if (professionId >= 0 && professionId < ProfessionType.COUNT) {
        return Player.professions[professionId];
    }
    return 0;
}
...
Player.packetUpgradeProfession <- function(professionId) {
    local packet = Packet();
    packet.writeUInt8(PacketId.Player);
    packet.writeUInt8(PacketPlayer.UpgradeProfession);
    packet.writeInt16(professionId);
    packet.send(RELIABLE_ORDERED);
    packet = null;
}
...
Player.onPacket <- function(packet)
{
    ...
    switch(packetType)
    {
        case PacketPlayer.Register:
			...
			ShowChat(true);
            callEvent("onPlayerLoggin", packet.readInt16());
        break;
        ...
        break;
        case PacketPlayer.UpdateLP:
            Player.learningPoints = packet.readInt32();
            callEvent("onClientLPUpdate", Player.learningPoints);
        break;
    }
}
...
addEventHandler("onPlayerShoot", function(heroId, munition) 
{
