...
{
    // Sprawdź wszystkie sloty ekwipunku używając dostępnych funkcji
    local equippedItems = [
        getPlayerArmor(pid),
...
function Database::createPlayer(player)
{
    ...
    // Zapisz przedmioty (również informację o założonych)
...
function Database::loadItems(pid)
{
    try {
        ...
        // Najpierw daj wszystkie przedmioty, potem załóż te które były założone
        foreach (item in items) {
            if (item.equipped == 1) {
