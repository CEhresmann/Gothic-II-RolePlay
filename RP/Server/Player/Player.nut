...
addEventHandler("onPlayerDead", function(playerid, killerid) {
    cancelEvent();
    playAni(playerid, "T_DEADB");
    setPlayerRespawnTime(playerid, 0);

    local packet = Packet();
    packet.writeUInt8(PacketId.Other);
    packet.writeUInt8(PacketOther.BW);
    packet.writeUInt8(1);
    packet.writeInt32(CFG.RespawnTime);
    ...
    if (packetId == PacketId.Other) {
        ...
        if (subPacketId == PacketOther.BW) {
            ...
            if (notificationType == 4) {
                local decision = packet.readUInt8();
                handleBWDecision(playerid, decision);
            }
        }
    }
});

function handleBWDecision(playerid, decision) {
    if (decision == 1) {
        local pos = getPlayerPosition(playerid);
        local ang = getPlayerAngle(playerid);
        
        respawnPlayer(playerid);
        
        setPlayerPosition(playerid, pos.x, pos.y, pos.z);
        setPlayerAngle(playerid, ang);
        
        setPlayerHealth(playerid, 10);
        
    } else if (decision == 2) {
        respawnPlayer(playerid);
        setPlayerHealth(playerid, Player[playerid].hpMax);
        
        local randomIndex = rand() % CFG.RESPAWN_POINTS.len();
        local respawnPoint = CFG.RESPAWN_POINTS[randomIndex];
        
        setPlayerPosition(playerid, respawnPoint.x, respawnPoint.y, respawnPoint.z);
    }
...
function updatePlayerBWTime(playerid, newTime) {
    local packet = Packet();
    packet.writeUInt8(PacketId.Other);
    packet.writeUInt8(PacketOther.BW);
    packet.writeUInt8(2);
    packet.writeInt32(newTime);
    packet.send(playerid, RELIABLE_ORDERED);
}
