...
class TwoH extends Bot
{
    ...
    function update()
    {
        switch(status)
        {
            case TWOH_STATUS.SPAWN:
                respawnAction();
            break;
            case TWOH_STATUS.SEARCH:
                searchAction();

                if(schedule && status == MONSTER_STATUS.SEARCH)
                    schedule.update();
            break;
            case TWOH_STATUS.RUN:
                runAction();
            break;
            case TWOH_STATUS.ATTACK:
                attackAction();
            break;
            case TWOH_STATUS.BACK:
                backAction();
            break;
        }
    }
    ...
    function runAction()
    {
        ...
        if(isOutAreaHome())
        {
            ...
            status = TWOH_STATUS.BACK;
            return;
        }

        if(isInArea(target, 300)) {
            status = TWOH_STATUS.ATTACK;
            timer = 1;
            attackAction();
            return;
        }
    ...
    function attackAction()
    {
        ...
        if(animation == runAnimation) {
            hitTarget(target, TWOH_ATTACK.MOVE);
            animation = "S_RUN";
        }
        ...
        {
            case 1: case 2:
                hitTarget(target, TWOH_ATTACK.NORMAL);
            break;
            case 4: case 5: case 6:
                hitTarget(target, TWOH_ATTACK.LEFT);
            break;
            case 8: case 7: case 9:
                hitTarget(target, TWOH_ATTACK.RIGHT);
            break;
...
function createTwoH(_type, x,y,z)
{
    local bot = TwoH(_type);
