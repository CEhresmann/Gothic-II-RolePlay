...
function handleTradeClose(senderId, targetId, traderId) {
    ...
    cancelTrade(traderId, partnerId, getPlayerName(traderId) + " anulowaæ handel.");
}
...
function handleDirectTradeResponse(playerId, traderId, accepted, responderId) {
    ...
    if (!isPlayerConnected(traderId) || !isPlayerSpawned(traderId)) {
        SendSystemMessage(playerId, "Gracz nie jest dostêny", {r=255, g=0, b=0});
        return;
    }
...
setTimer(function() {
    ...
    foreach (playerId in toRemove) {
        if (playerId in tradeRequests) {
            ...
            if (!isHandled) {
                SendSystemMessage(playerId, "Czas na odpowied¿ wygas³", {r=255, g=0, b=0});
                if (isPlayerConnected(targetId)) {
                    SendSystemMessage(targetId, "Czas na odpowied¿ wygas³", {r=255, g=0, b=0});
                }
            }
        }
    }
