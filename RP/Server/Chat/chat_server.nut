...
function sendNearbyMultiColorMessage(senderId, chatMode, prefix, prefixColor, content, contentColor, distance) {
    ...
    // Lista graczy, którzy faktycznie otrzymali wiadomoœæ
    local recipients = [];
    ...
    // Wyœlij powiadomienia tylko do graczy, którzy otrzymali wiadomoœæ i s¹ w innym trybie
    local notifyPacket = createNewMessageNotify(chatMode);
    local notifyP = Packet();
    notifyP.writeUInt8(PacketId.Other);
    notifyP.writeUInt8(PacketOther.ChatMessage);
    writeChatPacket(notifyPacket, notifyP);

    foreach (recipient in recipients) {
        if (recipient != senderId && (recipient in playerChatMode) && playerChatMode[recipient] != chatMode) {
            notifyP.send(recipient, RELIABLE);
        }
    }
}
...
function sendGlobalMessage(message, prefix = "", prefixColor = {r=255, g=255, b=255}, messageColor = {r=255, g=255, b=255}, chatMode = "IC") {
    local packet = createDisplayMultiColorChatMessage(chatMode, prefix, prefixColor, message, messageColor);
    local p = Packet();
    p.writeUInt8(PacketId.Other);
    p.writeUInt8(PacketOther.ChatMessage);
    writeChatPacket(packet, p);
