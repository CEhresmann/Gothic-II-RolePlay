...
function isValidSkillUpgrade(pid, skillType) {
    if (!isNpc(pid) && pid in Player && Player[pid].loggIn) {
        ...
        if (getPlayerLearningPoints(pid) < cost) {
            addNotification(pid, "Nie masz wystarczająco punktów nauki! Potrzeba " + cost + "PN.");
            return false;
        }
        ...
        switch (skillType) {
            case SkillType.Health:
                if (getPlayerMaxHealth(pid) >= SKILL_CONFIG.Limits.Health) {
                    addNotification(pid, "Osiągnąłeś maksymalny poziom zdrowia!");
                    return false;
                }
                break;
            case SkillType.Mana:
                if (getPlayerMaxMana(pid) >= SKILL_CONFIG.Limits.Mana) {
                    addNotification(pid, "Osiągnąłeś maksymalny poziom many!");
                    return false;
                }
                break;
            case SkillType.Strength:
                if (getPlayerStrength(pid) >= SKILL_CONFIG.Limits.Strength) {
                    addNotification(pid, "Osiągnąłeś maksymalny poziom siły!");
                    return false;
                }
                break;
            case SkillType.Dexterity:
                if (getPlayerDexterity(pid) >= SKILL_CONFIG.Limits.Dexterity) {
                    addNotification(pid, "Osiągnąłeś maksymalny poziom zręczności!");
                    return false;
                }
                break;
            case SkillType.OneHanded:
            ...
            case SkillType.Crossbow:
                ...
                if (getPlayerSkillWeapon(pid, weaponMap[skillType]) >= SKILL_CONFIG.Limits.WeaponSkill) {
                    addNotification(pid, "Osiągnąłeś maksymalny poziom tej umiejętności!");
                    return false;
                }
                break;
            case SkillType.MagicCircle:
                 ...
                 if (currentMagic == 0) {
                    addNotification(pid, "Musisz najpierw nauczyć się podstaw magii!");
                    return false;
                 }
                 if (currentMagic >= SKILL_CONFIG.Limits.MagicCircle) {
                    addNotification(pid, "Osiągnąłeś maksymalny krąg magiczny!");
                    return false;
                 }
                break;
            default:
                addNotification(pid, "Nieznany typ umiejętności!");
                return false;
        }
        return true;
    }
...
function isValidProfessionUpgrade(pid, professionType) {
    if (!isNpc(pid) && pid in Player && Player[pid].loggIn) {
        ...
        if (currentLevel >= PROFESSION_CONFIG.MaxLevel) {
            addNotification(pid, "Osiągnąłeś maksymalny poziom tej profesji!");
            return false;
        }
