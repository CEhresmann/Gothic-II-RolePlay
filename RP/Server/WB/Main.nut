...
function WorldBuilder::commandInit(pid, params)
{
    ...
    if (!args) {
        sendMessageToPlayer(pid, 255, 0, 0, "WB: Wpisz /wb [has³o]");
        return;
    }
    if(args[0] != CFG.WorldBuilderPassword) {
        sendMessageToPlayer(pid, 255, 0, 0, "WB: Z³e has³o do World Buildera.");
        return;
    }
...
function WorldBuilder::onPacket(pid, packet)
{
    ...
    if (packetId == PacketWorldBuilder.Vob) {
        ...
        try {
            existingVob = VobModel.findOne("name = ? AND x = ? AND y = ? AND z = ?", 
                [name, posx, posy, posz]);
        } catch (e) {}
